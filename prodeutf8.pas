...
procedure barraCualquiera(color:byte; linea: string);//Variacion de barraTitulo, para solucionar un pequeño bug cuando se usa junto con readkey...
    var
        i: byte;
    begin
...
procedure listarPartidos();
    ...
    begin
        ...
        if NOT(FileExists(ARCHIVO_FIXTURE)) then begin
            ...
            writeln();
            TextColor(Red);
            writelnCentrado('Error: todavía no hay un fixture cargado que mostrar.');
            writeln();
            writeln();
            ...
            writelnCentrado('Esta puede regenerarse con la opción 1 del menú de Administrador.');
            writeln();
        end else begin
        ...
        writeln();
        TextColor(LightGray);
        write('  Presione cualquier tecla para volver al menú anterior...');
        readkey;
    end;
procedure cargarResultado();
    ...
    begin
        ...
        if NOT(FileExists(ARCHIVO_FIXTURE)) then begin
            ...
            repeat
                repetir:= false;
                write('  Ingrese el número de partido a cargar: ');
                readln(input);
                ...
                if n=-2 then begin
                    ...
                    writeln();
                    writeln();
                    writelnCentrado('Uno o ambos valores fueron envíados en blanco. Operación cancelada.');
                    writeln();
                    TextColor(LightGray);
                    writeln();
                    ...
                    k:= teclaConsulta(C_ROJO,'Número de partido fuera de rango, ¿desea ingresar uno nuevamente?');
                    if k in ['s','S'] then begin
                        repetir:=true;
                        ClrScr;
                        printLogoMenu();
                    ...
                    repeat
                        ...
                        if NOT((i = -99) or (j = -99)) then begin
                            writeln('  Usted ingresó el siguiente resultado: ',partidos[n].local,' ',i,' - ',partidos[n].visitante,' ',j);
                            conf:= false;
                            ...
                            if k in ['s','S'] then begin
                                ...
                                writeln('    Resultado guardado con éxito.');
                                TextColor(LightGray);
                                writeln();
                            end else begin
                                conf:= true;
            ...
            if max=1 then begin
                ...
                while (i <= max) and (usuarios[i].nombre<>'NULLUSER') do begin
                    ...
                    if (i < max) AND (i mod 10 = 1) then begin
                        ...
                        writeln();
                        TextColor(C_AMARILLO);
                        write('  Presione cualquier tecla para mostrar la siguiente página...');
                        TextColor(LightGray);
                        readkey;
                        ClrScr;
                        printLogoMenu();
    ...
    begin
        ...
        if NOT(FileExists(ARCHIVO_FIXTURE)) then begin
            ...
            if (maxalcanzado) then begin
                TextColor(Red);
                writelnCentrado('Ya se alcanzó el límite máximo de usuarios para el sistema.');
                writeln();
                writeln();
            end else begin
                TextColor(White);
                ...
                if UpperCase(usuarioIng)='NULLUSER' then begin
                    ...
                    nickenuso:=false;{Acá se revisa que no exista el nombre de usuario en la DB}
                    ...
                    if nickenuso then begin
                        ...
                        if Length(usuarioIng)>10 then begin {Revisar largo del nombre ingresado}
                            ...
                            writelnCentrado('Solo pueden usarse caracteres alfanuméricos [a-Z,0-9]');
                            writeln();
                            ...
                            if agregado then begin
                                Rewrite(f);
                                Write(f, usuarios);
                                s:= 'Usuario '+usuarioIng+' creado con éxito.';
                                TextColor(LightGreen);
                                ...
                                TextColor(Red);
                                writelnCentrado('Error inesperado. No hay mas lugar, o algo raro pasó.');
...
procedure cargadorFixture();
    ...
    begin
        ...
        if NOT(FileExists(ARCHIVO_FIXTURE)) or NOT(FileExists(ARCHIVO_RESULTADOS)) then begin
            ...
            if NOT(FileExists(ARCHIVO_FIXTURE)) then begin
                resetearFixture();
                writelnCentrado('¡Fixture cargado con éxito!');
                j:= j+1;
            end;
            if NOT(FileExists(ARCHIVO_RESULTADOS)) then begin
                resetearResultados();
                writelnCentrado('¡Grilla Central creada con éxito!');
                j:= j+1;
            end;
...
procedure modifPartido(id: integer);//WIP
    ...
    begin
        ...
        if NOT(FileExists(ARCHIVO_FIXTURE)) then begin
            ...
            repeat
                repetir:= false;
                write('  Ingrese el número de partido a modificar: ');
                readln(input);
                n:= StrToIntDef(input,-1);
                if (length(input) = 0) then n:=-2;
                writeln();
                if n=-2 then begin
                    k:= teclaConsulta(C_ROJO,'No se ingresó ningún número de partido. ¿Desea ingresar uno nuevamente?');
                    if k in ['s','S'] then begin
                        repetir:= true;
                        ClrScr;
                        printLogoMenu();
                    ...
                    repeat
                        ...
                        write('    Partido ',n,': ',partidos[n].local,' - ',partidos[n].visitante);
                        TextColor(C_GRIS);
                        if (usuarios[id].prode[n].gol_l < 0) or (usuarios[id].prode[n].gol_v < 0) then writeln() else writeln(' (Predicción actual: ',usuarios[id].prode[n].gol_l,' - ',usuarios[id].prode[n].gol_v,')');
                        TextColor(LightGray);
                        writeln();
                        TextColor(Green);
                        repeat    
                        ...
                        writeln();
                        if NOT((i = -99) or (j = -99)) then begin
                            writeln('  Usted ingresó la siguiente predicción: ',partidos[n].local,' ',i,' - ',partidos[n].visitante,' ',j);
                            conf:= false;
                            k:= teclaConsulta(C_CYAN,'¿Es esto correcto?');
                            if k in ['s','S'] then begin
                                conf:= true;
                                ...
                                writeln();
                                TextColor(LightGreen);
                                writeln('    Predicción guardada con éxito.');
                                TextColor(LightGray);
                                writeln();
                            end else begin
                                conf:= true;
                                writeln();
                                writeln();
                                k:= teclaConsulta(C_AZUL,'¿Desea volver a ingresar la predicción?');
                                if k in ['n','N'] then salir:= true;
                            end;
                        end else begin
                            conf:= true;
                    ...
                    if NOT(salir) then begin
                        repetir:= false;
                        k:= teclaConsulta(C_AZUL,'¿Desea agregar otra predicción?');
                        if k in ['s','S'] then begin
                            repetir:= true;
                            ClrScr;
                            printLogoMenu();
...
procedure tablaPosiciones(id: integer);
    ...
    begin
        ...
        if NOT(FileExists(ARCHIVO_FIXTURE)) then begin
            ...
            completo:= false;
            writeln('  ┌───────────┬────────────┬───────────┐');
            writeln('  │ Ubicación │   Nombre   │  Puntaje  │');
            writeln('  ├───────────┼────────────┼───────────┤');
            while (i <= MAX_USUARIOS) and NOT(completo) do begin
                lineaTabla(usuarios,res,i,completo,usuarios[id].nombre);
                i:= i+1;
                if (completo) or (i mod PUESTOS_POR_HOJA_RANKING = 1) then writeln('  └───────────┴────────────┴───────────┘') else writeln('  ├───────────┼────────────┼───────────┤');
    ...
    begin
        ...
        repeat
            repeat
                ...
                barraTitulo('Por favor, ingrese el número correspondiente a la opción deseada.');
                writeln();
